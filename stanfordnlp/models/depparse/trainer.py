...
class Trainer(BaseTrainer):
    ...
    def __init__(self, args=None, vocab=None, pretrain=None, model_file=None, use_cuda=False):
        ...
        self.mapping_optimizer = utils.get_optimizer('rsgd', self.model.hypmapping.parameters(), 1.0)
    ...
    def update(self, batch, eval=False, subsample=True):
        ...
        if subsample:
            loss, edge_acc, preds= self.model(word, word_mask, wordchars, wordchars_mask, upos, xpos, ufeats, pretrained, lemma, head, deprel, word_orig_idx, sentlens, wordlens, self.scale, root, True)
        else:
            loss, edge_acc, preds= self.model(word, word_mask, wordchars, wordchars_mask, upos, xpos, ufeats, pretrained, lemma, head, deprel, word_orig_idx, sentlens, wordlens, self.scale, root, False)
    ...
    def predict(self, batch, unsort=True):
        ...
        self.model.eval()
        loss, edge_acc, preds = self.model(word, word_mask, wordchars, wordchars_mask, upos, xpos, ufeats, pretrained, lemma, head, deprel, word_orig_idx, sentlens, wordlens, self.scale, root)
